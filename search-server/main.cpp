...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate) const {
        //Функция ParseQuery выбросит исключение invalid_argument если:
        // - в словах поискового запроса есть недопустимые символы
        // – присутсвет наличие более чем одного минуса
        // – отсутвует текст после минуса
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
        //Функция ParseQuery выбросит исключение invalid_argument если:
        // - в словах поискового запроса есть недопустимые символы
        // – присутсвует наличие более чем одного минуса
        // – отсутвует текст после минуса
    ...
    Query ParseQuery(const string& text) const {
        ...
        for (const string& word : SplitIntoWords(text)) {
            ...
            if (query_word.data.empty()) {
                throw invalid_argument("Отсутсвие текста после символа \"минус\" в поисковом запросе"s );
            };
        }
        return query;
    }
